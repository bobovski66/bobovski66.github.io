...
def create_index_html(base_directory, summary_links):
    ...
    for idx, (summary_link, thumbnail) in enumerate(summary_links, start=1):
        # Ensure that both thumbnail and summary link paths are relative to the index.html file
        summary_directory = os.path.dirname(summary_link)
        relative_thumbnail_path = os.path.join(summary_directory, os.path.basename(thumbnail))
        relative_summary_link = summary_link
...
def traverse_and_analyze(base_directory):
    ...
    for dir_name in subdirectories:
        ...
        summary_html, thumbnail = run_analysis_in_directory(directory_path)
        if summary_html and thumbnail:
            summary_links.append((summary_html, thumbnail))
